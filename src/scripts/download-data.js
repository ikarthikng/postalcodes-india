...
// This script downloads the GeoNames IN ZIP code data file
...
// URL for the IN ZIP code data from GeoNames
const ZIP_DATA_URL = "https://download.geonames.org/export/zip/IN.zip"
...
const zipPath = path.join(dataDir, "IN.zip")
const txtPath = path.join(dataDir, "IN.txt")
...
async function extractZipFile(zipPath, txtPath) {
  ...
  return new Promise((resolve, reject) => {
    ...
    exec(`unzip -p ${zipPath} IN.txt > ${txtPath}`, (error) => {
      if (error) {
        ...
        try {
          ...
          import("unzipper")
            ...
            .catch((e) => {
              ...
              console.log("Please extract IN.txt from the downloaded ZIP file manually:")
              console.log(`1. Open ${zipPath}`)
              console.log(`2. Extract IN.txt to ${txtPath}`)
...
async function main() {
  try {
    ...
    if (fs.existsSync(txtPath)) {
      console.log(`Removing existing IN.txt file: ${txtPath}`)
    ...
    if (fs.existsSync(txtPath)) {
      console.log(`IN.txt has been extracted to ${txtPath}`)
      ...
      console.log("Extraction might have failed. Please extract IN.txt manually as described above.")
    }
  } catch (error) {
